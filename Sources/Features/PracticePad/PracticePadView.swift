...
struct PracticePadView: View {
    ...
    @State private var wordPosition: UInt8 = 0
    @State private var rhythm = RhythmBuffer()
    ...
    private func handle(_ keystroke: Keystroke) {
        ...
        case .wordDelete:
            keyClass = .backspace
            characterCount = max(0, characterCount - Int(wordPosition))
            wordPosition = 0
        }
    ...
    private func save() async {
        ...
        let saved = await model.saveSession(
            events: events,
            startedAt: startedAt,
            category: .writing,
            source: .practicePad
        ...
        if saved {
            ...
            saveMessage = "Type for a little longer before saving."
        }
    }
